...
export default function ShippingMethod({
  ...
  const onSubmit = async (data: FieldValues) => {
    if (!data?.method) {
      showToast("Por favor elige el método de envío", "warning");
      return;
    }
    try {
      await saveCheckoutShipping(data?.method);
    } catch {
      showToast("Error al guardar el método de envío. Por favor inténtalo de nuevo.");
    }
  };
          ...
          <>
            <div className="mt-4  justify-between hidden sm:flex">
              <div className="flex">
                ...
                <p className="text-base font-normal">{selectedMethodTitle} (Bs {selectedMethodPrice})</p>
              </div>
              <div className="flex">
                <button
