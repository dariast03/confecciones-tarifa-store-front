...
export const useCheckout = () => {
  ...
  const {
    ...
    onError: (error) => {
      showToast(error?.message || 'Algo salió mal', 'danger');
    },
  });
  ...
  const handleGetShippingRates = async () => {
    ...
    try {
      ...
      if (Array.isArray(response?.data)) {
        showToast('Tasas de envío cargadas', 'success');
      } else {
        showToast('No se encontraron tasas de envío', 'warning');
      }
  ...
  const { mutateAsync: saveShipping, isPending: isSaving } = useMutation({
    ...
    onSuccess: (response) => {
      const responseData =
        response?.data?.createCheckoutShippingMethod?.checkoutShippingMethod;
  ...
  const { mutateAsync: savePayment, isPending: isPaymentLoading } = useMutation(
    {
      ...
      onSuccess: (response) => {
        const responseData =
          response?.data?.createCheckoutPaymentMethod?.checkoutPaymentMethod;
