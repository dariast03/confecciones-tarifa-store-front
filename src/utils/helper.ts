...
export async function generateMetadataForPage(
  ...
  return {
    ...
    twitter: {
      ...
      description,
      images: [ogImage],
      creator: '@ConfeccionesTarifa',
      site: '@ConfeccionesTarifa',
    },
...
export const useAddressesFromApi = (): {
  ...
  const billingNode = address.find(
    (a) => a.node?.addressType === 'cart_billing',
  )?.node;
  ...
  const mapNode = (node?: CheckoutAddressNode): MappedCheckoutAddress | null =>
    node
      ? {
